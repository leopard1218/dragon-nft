...
contract DragonFarm is Ownable {
  ...
  constructor(
    ...
    dragonNFT = IDragonNFT(nftAddr);
    yieldToken = IDragonToken(ftAddr);
  }

  function batchStake(uint[] memory tokenIds) external payable {
    updateHarvest();
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(dragonNFT.ownerOf(tokenIds[i]) == msg.sender, 'you are not owner!');
      ownerOfToken[tokenIds[i]] = msg.sender;
      dragonNFT.transferFrom(msg.sender, address(this), tokenIds[i]);
  ...
  function batchWithdraw(uint[] memory tokenIds) external payable {    
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      require(ownerOfToken[tokenIds[i]] == msg.sender, "DragonFarm: Unable to withdraw");
      dragonNFT.transferFrom(address(this), msg.sender, tokenIds[i]);
  ...
  function withdrawFt(uint _amount) external payable {
    require(stakeBalancesFt[msg.sender] >= _amount, "DragonFarm: Unable to withdraw Ft");
    harvestFt();
  ...
  function getTotalClaimableFt(address _user) external view returns(uint256) {
    uint256 time = block.timestamp;
    uint256 pending = stakeBalancesFt[msg.sender] * (time - lastUpdateFt[_user]) / 86400 / 50;
